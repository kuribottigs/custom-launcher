//! Single-line text input, adapted from gpui's `input` example and themed
//! for the launcher. Supports selection, clipboard, and IME composition.
...
/// Register the key bindings used by [`TextInput`]. Call once at startup.
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), gpui::rgb(theme::TEXT_DIM).into())
        } else {
            (content, style.color)
        };
